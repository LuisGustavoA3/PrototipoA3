  ...
  return (
    <div className="space-y-5 p-6">
      ...
      <div className="grid gap-5 lg:grid-cols-[1.35fr_1.15fr]">
        ...
        <Card className="h-[430px]"><CardHeader title="Prioridades da jornada" action={<Link to="/jornada-de-desenvolvimento" className="label-caps rounded bg-primary px-2.5 py-1 text-[10px] text-primary-foreground">Ver jornada</Link>} />{journeyPriorities.length === 0 ? <EmptyState text="Nenhuma prioridade cadastrada." /> : <ul className="space-y-3">{journeyPriorities.slice(0, 8).map((priority) => <li key={priority.id}><Link to="/jornada-de-desenvolvimento" className="flex min-w-0 items-start gap-2 text-base text-foreground hover:text-primary"><span className="mt-1 shrink-0 text-primary">•</span><span className="min-w-0 truncate whitespace-nowrap pt-1">{priority.title}</span></Link></li>)}{journeyPriorities.length > 8 && <li className="pt-1 text-xs text-muted-foreground">+ {journeyPriorities.length - 8} prioridades de jornada</li>}</ul>}</Card>
      </div>
